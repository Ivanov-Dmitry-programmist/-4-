...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	middleRunningSpeed := meanSpeed(action, duration)
	calories := ((runningCaloriesMeanSpeedMultiplier * middleRunningSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return calories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	middleWalkingSpeed := meanSpeed(action, duration)
	calories := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(middleWalkingSpeed*0.278, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	middleSwimmingSpeed := swimmingMeanSpeed(lengthPool, countPool, duration)
